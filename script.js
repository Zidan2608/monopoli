 ...
 */
let players = [
    { name: "Pemain 1", points: CONFIG.STARTING_POINTS, position: 0, color: "player-1", hasAngelCard: false },
    { name: "Pemain 2", points: CONFIG.STARTING_POINTS, position: 0, color: "player-2", hasAngelCard: false },
    { name: "Pemain 3", points: CONFIG.STARTING_POINTS, position: 0, color: "player-3", hasAngelCard: false },
    { name: "Pemain 4", points: CONFIG.STARTING_POINTS, position: 0, color: "player-4", hasAngelCard: false },
];
...
const cities = [
    ...
    "Inggris", "Prancis", "Indonesia", "Australia"
];
 ...
 * Index 18: PENJARA (bottom-left, only pass)
 */
let boardTiles = [];
...
function initializeBoardTiles() {
    ...
    boardTiles = [
        ...
        { name: cities[cityIndex++], type: "question" },
        { name: "KESEMPATAN", type: "opportunity" },
        { name: cities[cityIndex++], type: "question" },
        { name: "DANA UMUM", type: "community" },
        { name: cities[cityIndex++], type: "question" },
        ...
        { name: "DANA UMUM", type: "community" },
        { name: cities[cityIndex++], type: "question" },
        { name: "PARKIR KEMANA SAJA", type: "parking" },
        { name: cities[cityIndex++], type: "question" },
        { name: "KESEMPATAN", type: "opportunity" },
        { name: cities[cityIndex++], type: "question" },
        { name: "DANA UMUM", type: "community" },
 ...
 */
const allQuestions = [
    // Soal 1-10: Etika Dasar
    { q: "Seorang akuntan diminta menutupi kerugian perusahaan dalam laporan keuangan. Apa yang harus dilakukan?", c: ["Menutupi kerugian untuk melindungi reputasi", "Melaporkan kerugian dengan jujur", "Menunda pelaporan untuk mencari solusi", "Hanya melaporkan sebagian kerugian"], a: 1, id: 1 },
    { q: "Klien memberikan hadiah berharga saat menandatangani kontrak. Sebaiknya...", c: ["Terima hadiah sebagai bentuk apresiasi", "Tolak hadiah untuk menjaga independensi", "Terima tapi kembalikan setengahnya", "Tanyakan ke atasan terlebih dahulu"], a: 1, id: 2 },
 ...
 * @param {string} cityName - Name of city to update marker for
 */
function updateCityOwnershipDisplay(cityName) {
    const ownerIndex = cityOwnership[cityName];
    if (ownerIndex === undefined) return;
    
    boardTiles.forEach((tile, index) => {
        if (tile.name === cityName) {
...
function createBoard() {
    ...
    Object.keys(cityOwnership).forEach(updateCityOwnershipDisplay);
}
...
function createCardBoxes() {
    ...
    cardContainer.style.cssText = "display: grid; grid-template-columns: 1fr 1fr; gap: 10px; padding: 10px;";
    ...
    opportunityBox.innerHTML = `
        ...
        <div class="card-box-count">8 Kartu</div>
    `;
...
function eliminatePlayer(index, reason) {
    ...
    Object.keys(cityOwnership).forEach((city) => {
        const owner = cityOwnership[city];
        if (owner === index) {
            delete cityOwnership[city];
        } else if (owner > index) {
            cityOwnership[city] = owner - 1;
        }
    });
...
function handleTileEffect(position) {
    ...
    switch (tile.type) {
        ...
        case "jail":
            if (position === 18) {
                logMessage(`🚶 ${player.name} melewati PENJARA`);
                updateGameInfo(`Anda melewati penjara`);
                nextPlayer();
            } else {
                handleJail();
            }
...
function handleJail() {
    ...
    if (!state.inJail) {
        ...
        updateGameInfo(`🔒 Anda masuk PENJARA! Bayar ${CONFIG.JAIL_PAYMENT_COST} poin ATAU lempar dadu untuk dapat 6.`);
        showJailOptions();
    }
}
...
function showJailOptions() {
    ...
    qText.innerHTML = `<div style="font-size:18px; color:#d32f2f; font-weight:bold;">🔒 Di PENJARA</div>
    ...
    diceBtn.onclick = () => {
        ...
        logMessage(`🎲 ${player.name} lempar dadu: ${roll}`);
        
        if (roll === 6) {
            jailState[currentPlayerIndex].inJail = false;
            jailState[currentPlayerIndex].turnsInJail = 0;
            logMessage(`✅ ${player.name} dapat 6! Keluar dari penjara`);
            updateGameInfo(`Dapat 6! Anda keluar penjara dan bergerak ${roll} langkah`);
            movePlayer(currentPlayerIndex, roll);
        } else {
            jailState[currentPlayerIndex].turnsInJail++;
            logMessage(`❌ ${player.name} dapat ${roll}, tetap di penjara (giliran ke-${jailState[currentPlayerIndex].turnsInJail})`);
            updateGameInfo(`Dapat ${roll}, tidak cukup. Tetap di penjara. (Giliran ke-${jailState[currentPlayerIndex].turnsInJail})`);
 ...
 * @private
 */
function showQuestionForFine(cityName) {
    const selectedQuestion = getUnusedQuestion();
...
function handleBuyAnswerResult(player, isCorrect) {
    if (isCorrect) {
        ...
        cityOwnership[cityName] = currentPlayerIndex;
        updateCityOwnershipDisplay(cityName);
...
// ===== CARD SYSTEMS =====
/**
 * Opportunity cards (KESEMPATAN) - special event cards
 * Structure: {msg, money?, score?, move?, goTo?, bonus?}
 * @type {Array<Object>}
 */
const opportunityCards = [
    { msg: "🎁 Anda memenangkan kontes! +200 poin", money: 200, score: 0 },
    { msg: "💸 Anda kena denda parkir! -100 poin", money: -100, score: 0 },
    { msg: "⭐ Bonus pengetahuan! +10 poin", money: 0, score: 10 },
    { msg: "🏠 Maju 3 langkah!", move: 3 },
    { msg: "🔙 Mundur 2 langkah!", move: -2 },
    { msg: "💰 Dividen dari investasi! +150 poin", money: 150, score: 0 },
    { msg: "🎯 Pergi ke START! +200 poin", goTo: 0, bonus: 200 },
    { msg: "🚂 Pergi ke kota terdekat!", move: "nearestCity" },
];

/**
 * Community cards (DANA UMUM) - community chest cards
 * @type {Array<Object>}
 */
const communityCards = [
    { msg: "🎁 Hadiah ulang tahun! +100 poin", money: 100, score: 0 },
    { msg: "🏥 Bayar biaya kesehatan! -50 poin", money: -50, score: 0 },
    { msg: "🎓 Beasiswa pendidikan! +150 poin dan +5 poin", money: 150, score: 5 },
    { msg: "🏛️ Pajak properti! -75 poin", money: -75, score: 0 },
    { msg: "💰 Warisan dari keluarga! +200 poin", money: 200, score: 0 },
    { msg: "🎯 Pergi ke PENJARA!", goTo: 6 },
    { msg: "🏠 Pergi ke START!", goTo: 0, bonus: 200 },
    { msg: "⭐ Penghargaan prestasi! +15 poin", money: 0, score: 15 },
];

/**
 * Handle opportunity/chance card landing
 * Draws random opportunity card and applies effect
 * @private
 */
function handleOpportunity() {
    const player = players[currentPlayerIndex];
    player.hasAngelCard = true;
    logMessage(`👼 ${player.name} mendapat kartu KESEMPATAN! Tidak akan kena pajak/denda pada giliran berikutnya.`);
    updateGameInfo(`👼 Anda mendapat kartu KESEMPATAN! Tidak akan kena pajak/denda pada giliran berikutnya.`);
    updateUI();
    nextPlayer();
}
 ...
 * Applies points/movement based on card type
 * @param {number} cardIndex - Index of opportunity card to execute
 * @private
 */
function executeOpportunityCard(cardIndex) {
    const player = players[currentPlayerIndex];
    const opportunity = opportunityCards[cardIndex];
    
    if (opportunity.move) {
        if (opportunity.move === "nearestCity") {
            let nearestCityPos = player.position;
            for (let i = 1; i < CONFIG.BOARD_SIZE; i++) {
                const checkPos = (player.position + i) % CONFIG.BOARD_SIZE;
                if (boardTiles[checkPos].type === "question") {
                    nearestCityPos = checkPos;
                    break;
                }
            }
            movePlayerToPosition(currentPlayerIndex, nearestCityPos);
        } else {
            const newPos = (player.position + opportunity.move + CONFIG.BOARD_SIZE) % CONFIG.BOARD_SIZE;
            movePlayerToPosition(currentPlayerIndex, newPos);
        }
        logMessage(`🎲 ${player.name}: ${opportunity.msg}`);
        updateGameInfo(opportunity.msg);
    } else if (opportunity.goTo !== undefined) {
        movePlayerToPosition(currentPlayerIndex, opportunity.goTo);
        player.points += (opportunity.bonus || 0) + (opportunity.score || 0) + (opportunity.money || 0);
        logMessage(`🎲 ${player.name}: ${opportunity.msg}`);
        updateGameInfo(opportunity.msg);
    } else {
        player.points += (opportunity.money || 0) + (opportunity.score || 0);
        logMessage(`🎲 ${player.name}: ${opportunity.msg}`);
        updateGameInfo(opportunity.msg);
    }
...
function executeCommunityCard(cardIndex) {
    ...
    if (card.goTo !== undefined) {
        ...
        if (card.bonus) player.points += card.bonus;
        player.points += (card.score || 0) + (card.money || 0);
        logMessage(`📦 ${player.name}: ${card.msg}`);
        updateGameInfo(card.msg);
    } else {
        player.points += (card.money || 0) + (card.score || 0);
        logMessage(`📦 ${player.name}: ${card.msg}`);
        updateGameInfo(card.msg);
    }
...
// ===== MODAL SYSTEM =====
/**
 * Show card modal with random card selection
 * Displays card message and apply button
...
function showCardModal(cardType, cards) {
    ...
    if (cardType === "KESEMPATAN") {
        header.innerHTML = `<h2>🎲 KESEMPATAN</h2>`;
        header.className = "card-header opportunity-header";
    } else {
        header.innerHTML = `<h2>📦 DANA UMUM</h2>`;
    ...
    instructionDiv.textContent = "Klik tombol di bawah untuk menerapkan efek kartu";
